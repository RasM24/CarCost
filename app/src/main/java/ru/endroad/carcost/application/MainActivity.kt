...
class MainActivity : ComponentActivity() {
	...
	private fun handleAction(action: SelectionHubAction, navController: NavController) {
		when (action) {
			...
			SelectionHubAction.CivicServiceBookClicked   -> Unit//router.openCivicServiceBookScreen()
			SelectionHubAction.StepwgnCatalogClicked     -> navController.navigate(CATALOG_STEPWGN_DESTINATION)
			SelectionHubAction.StepwgnServiceBookClicked -> Unit//router.openStepwgnServiceBookScreen()
		}
	}
}
